...
class TestListDrives:
    ...
    def test_invalid_drive(self, client):
        c, db, drive_dir, data_dir = client
        res = c.get("/api/drives/nonexistent/videos")
...
class TestListFolders:
    def test_root_folders(self, client):
        ...
        assert "旅行" in names
        assert "料理" in names
        assert all(f["video_count"] == 1 for f in folders)
...
class TestListDriveVideos:
    def test_all_videos(self, client):
        c, db, drive_dir, data_dir = client
        _seed(db, drive_dir)
    ...
    def test_filter_by_path(self, client):
        c, db, drive_dir, data_dir = client
        _seed(db, drive_dir)
        res = c.get(f"/api/drives/{TEST_DRIVE}/videos?path=旅行")
        assert res.status_code == 200
    ...
    def test_filter_by_path_no_match(self, client):
        c, db, drive_dir, data_dir = client
        _seed(db, drive_dir)
        res = c.get(f"/api/drives/{TEST_DRIVE}/videos?path=音楽")
        assert res.status_code == 200
    ...
    def test_search(self, client):
        c, db, drive_dir, data_dir = client
        _seed(db, drive_dir)
        res = c.get(f"/api/drives/{TEST_DRIVE}/videos?search=V")
        assert len(res.json()["data"]) == 2

        res = c.get(f"/api/drives/{TEST_DRIVE}/videos?search=nonexistent")
        assert len(res.json()["data"]) == 0

    def test_sort(self, client):
        c, db, drive_dir, data_dir = client
        _seed(db, drive_dir)
        res = c.get(f"/api/drives/{TEST_DRIVE}/videos?sort=title&order=asc")
        assert res.status_code == 200

    def test_pagination(self, client):
        c, db, drive_dir, data_dir = client
        _seed(db, drive_dir)
        res = c.get(f"/api/drives/{TEST_DRIVE}/videos?page=1&limit=1")
        assert res.status_code == 200
        body = res.json()
