...
def _seed_video(db, drive_dir, suffix=""):
    folder = drive_dir / "旅行"
...
class TestListTags:
    ...
    def test_tag_count(self, client):
        c, db, drive_dir, data_dir = client
        v1 = _seed_video(db, drive_dir, "1")
        v2 = _seed_video(db, drive_dir, "2")
...
class TestTagFilter:
    def test_filter_by_tag(self, client):
        ...
        res = c.get(f"/api/drives/{TEST_DRIVE}/videos?tag=night")
    ...
    def test_filter_no_match(self, client):
        ...
        res = c.get(f"/api/drives/{TEST_DRIVE}/videos?tag=nonexistent")
