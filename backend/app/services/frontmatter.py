...
decides how to react — broken YAML should not block a save.
"""
...
# Aliases are user-facing labels with weaker character constraints than tags
# (e.g. CJK and spaces are common). The caps are advisory — alias storage is
...
def parse(content: str) -> ParsedMarkdown:
    ...
    function performs no length check; an unbounded caller could trigger
    pathological ``split`` memory use or deep YAML recursion.
    """
