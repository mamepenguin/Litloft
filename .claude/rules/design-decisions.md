...
- **`drives.json.addons.<name>` is not that switch.** It filters which addons and slots a *drive* reports (`main.py`'s `enabled_names`), so the UI hides them; the router is still mounted and its endpoints still answer. Use it to scope a feature to some drives, not to unload an addon.
