...
- **An in-process addon is enabled by being checked out, and that is the only durable switch.** `backend/addons/` is what `_load_addons` walks (`pkgutil.iter_modules`), and it is generated: `setup-addons.sh` links every addon under `addons/` into it, and `backend/Dockerfile` deletes any link and copies each addon's `backend/` in as a real directory. So removing the symlink disables the addon only until the next `setup-addons.sh` run recreates it, and in a built image there is no symlink to remove at all. Not checking out the submodule is what actually turns one off, in both layouts. (This previously read "enable/disable is controlled by adding/removing a symlink". Measured against `57f65ecf`, before the link tree existed: the pre-existing script recreated a removed backend link on its very next run, so the sentence was already describing something that did not hold.) Do not modify core code to disable an addon.
- **`drives.json.addons.<name>` is not that switch either, and what it closes depends on which surface you mean.** It is not an unload: nothing is unmounted, and `main.py`'s `enabled_names` only filters which addons and slots a *drive* reports, so the UI hides them. But the two surfaces then differ, and the difference is measured, not assumed:
  - **An in-process addon's own routes keep answering.** `_load_addons` calls `app.include_router` unconditionally at import time; the policy is applied later and only to the listing.
  - **A proxied route closes with 404**, which is the two-layer defence stated above. Both paths end in `_require_addon_feature`, which raises 404 rather than 403 so a disabled route is not revealed, and a manifest reaches it two ways on purpose: a `pre_check` of `{type: addon_feature}`, and an `addon_feature` key on the route itself for file-scoped routes, which must compose the feature gate with the core file-access check and cannot use `pre_check`'s single discriminator (`addon_proxy.py:568-577`).

  So "policy-off means the data is unreachable" holds for the proxied surface and not for an in-process router. Use this to scope a feature to some drives; to stop an addon loading at all, do not check it out.
