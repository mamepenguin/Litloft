...
- **500ms debounce**: chip 編集の content PUT は `createDebouncedTagSaver` で畳む。spec §D7 は当初 editor auto-save と合わせて 2s を選んだが、chip 編集は keystroke ストリームではなく離散確定（Enter / × クリック）なので 2s は長すぎた。500ms は「追加 → タイポ訂正 → 再追加」級の連続訂正を 1 リクエストに束ねつつ LAN 反映体感 ~0.6s で即時に感じられる設定。保存成功後に sidebar refresh 等の side effect は `onSaveSuccess` コールバック経由で post-save に遅延
