 ...
 * same place, which is what keeps the two `⋮` columns lined up; and how
 * much of the row the name gets, against the same row drawn the way it was
 * drawn before.
 *
 * ## What it cannot see
 *
...
const WIDTHS = [
  {
    ...
    link: { grouped: 245, separated: 227, fine: 259 },
    name: { grouped: 137, separated: 119, fine: 151 },
  },
  {
    width: 288,
    pad: PAD_PX,
    label: "the row on a 320px phone",
    link: { grouped: 190, separated: 172, fine: 204 },
    name: { grouped: 82, separated: 64, fine: 96 },
  },
  {
    width: 700,
    pad: PAD_SM_PX,
    label: "a row above the sm breakpoint",
    link: { grouped: 604, separated: 588, fine: 620 },
...
async function measure(page: import("@playwright/test").Page, width: number) {
  ...
  expect(Object.keys(by).sort()).toEqual([...ALL_SHAPES].sort());
  return { m, by };
}
...
test.describe("with a finger", () => {
  ...
  for (const w of WIDTHS) {
    ...
    test(`groups the trailing controls at ${w.width}px — ${w.label}`, async ({
      ...
      // 7. Selection mode draws no trailing control, so it keeps its
      //    trailing padding. Dropping the padding unconditionally would
      //    leave this row's content against the edge with nothing there
      //    to have earned it.
      expect(by.selectable.controls).toHaveLength(0);
      expect(by.selectable.row.right - by.selectable.link.right).toBe(w.pad);
...
test.describe("with a mouse", () => {
  ...
  for (const w of WIDTHS) {
    ...
    test(`leaves the fine-pointer row alone at ${w.width}px`, async ({
      ...
      // front of the group, and its trailing padding.
      ...
      // So all a mouse sees of this change is the gap that used to sit
      // between the two controls, which the group no longer draws.
      expect(file.link.width).toBe(w.link.fine);
      expect(file.link.width).toBe(
        w.width - w.pad * 2 - GAP_PX - STAR_FINE_PX - MORE_FINE_PX,
      );
      if (w.name) {
