...
describe("useInspectorOpen", () => {
  it("defaults to open on wide viewport (>= 1280px) when no localStorage", () => {
    setViewportWidth(1440);
    const { result } = renderHook(() => useInspectorOpen("work"));
    expect(result.current.open).toBe(true);
  });

  it("defaults to closed on narrow viewport (< 1280px) when no localStorage", () => {
    setViewportWidth(1100);
    const { result } = renderHook(() => useInspectorOpen("work"));
    expect(result.current.open).toBe(false);
  });

  it("treats the 1280px boundary as open (>= threshold)", () => {
