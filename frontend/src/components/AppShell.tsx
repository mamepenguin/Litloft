      ...
      <QuickNoteProvider>
        <GlobalSearchProvider>
          <div className="min-h-dvh">
            <Sidebar />
            {/* One row, so the tree toggle is placed by the menu button rather
                than by a copy of its geometry.

                PWA safe-area: when iOS runs in standalone mode the viewport
                can extend under the status bar, so the row is anchored to the
                safe-area top + 12px instead of a raw 12px offset. In a
                regular browser `safe-area-inset-top` resolves to 0. */}
            <div
