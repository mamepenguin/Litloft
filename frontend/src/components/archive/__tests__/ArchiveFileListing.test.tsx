...
describe("ArchiveFileListing", () => {
  ...
  it("calls handleBreadcrumbClick on breadcrumb click", () => {
    const handleBreadcrumbClick = vi.fn();
    render(
      <ArchiveFileListing {...defaultProps} handleBreadcrumbClick={handleBreadcrumbClick} />
    );
  ...
  it("disables non-clickable entries", () => {
    ...
    // Breadcrumb buttons are not disabled, entry buttons should be
    const entryButtons = buttons.filter((b) => b.hasAttribute("disabled"));
    // All 3 entries should be disabled (dirs are always clickable via handleDirClick, but button itself is disabled per isClickable)
