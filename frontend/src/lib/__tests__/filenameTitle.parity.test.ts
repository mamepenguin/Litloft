 ...
 * case list has to be the *same* file, not two copies, or the drift moves
 * into the fixtures.
 */
...
describe("filenameToTitle agrees with the backend", () => {
  it("reads the whole table, not some of it", () => {
    // The number is also written down on the Python side, so deleting a
    // case is a two-file edit.
