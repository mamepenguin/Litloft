/**
 * A count is drawn only when the readout, the arrows and the listing are
 * the same sequence. A wrong `N` says a file is there that nothing can
 * reach.
 *
...
describe("resolveFileNavOrdering", () => {
  ...
  it("takes the order from the URL, which is where the listing put it", () => {
    // Not `folderPrefs`: the drive root never writes an entry, and the
    // full-screen gallery reads the ordering from the URL.
