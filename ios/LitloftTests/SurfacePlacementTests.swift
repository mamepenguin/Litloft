...
struct SurfacePlacementTests {
    ...
    func scrollerScroll() {
        ...
        #expect(place(inner, document: 500, scroller: 0)?.minY == 80)
        #expect(place(inner, document: 500, scroller: 100)?.minY == -20)
    }
    ...
    func fixedFrame() {
        #expect(place(geometry(anchor: .fixed, top: 0), document: 400, scroller: 90)?.minY == 0)
    }
    ...
    func stickyInScroller() {
        ...
        let sticky = geometry(anchor: .scroller(box), top: 24, stick: SurfaceGeometry.Stick(top: 56, limit: 1200))
        #expect(place(sticky, scroller: 0)?.minY == 80)
        #expect(place(sticky, scroller: 400)?.minY == 56)
        // 56 + 1200 - 1100 - 225
        #expect(place(sticky, scroller: 1100)?.minY == -69)
    }
