...
- **Note search** — on a note's screen, `Cmd/Ctrl+K` and the header's search button open search narrowed to the drive's notes.
...
`edit=1`. New note is shown whatever the `editor` feature says, because Quick
Note does not need the editor.
...
follow that file's folder, not the Notes order. While a Markdown or text file
is on screen, the addon registers a search scope from the action row
(`file-detail-actions`, which renders nothing): the core search modal opens
scoped to notes (`{ type: "text" }`, labelled Notes, with a see-all link to
`?view=all&q={query}`); see
...
The **Add** menu's **Clip web page** row sends the same `POST /clips` into the folder the menu was opened from; it does not use the folder the Notes page last remembered. If the URL was clipped before and that clip did not fail, the duplicate prompt opens (**Open existing**, **Create new**, **Cancel**); a URL whose earlier clips all failed is clipped again without asking. Once the clip is accepted the dialog and menu close, and a toast reports the result: "Clipped: {title}" when it is ready, an error when it fails. Only clips sent from the Add menu in the current tab are announced; clips sent from the Notes page stay in its clip history as before and are not added there from the Add menu.

Two known gaps: a notification can be missed when another live update arrives at the same moment (the clip itself is still created and appears on the Notes page), and if you close the dialog before the duplicate lookup answers, nothing is clipped and nothing reports it. If you close it while the clip request is being sent and that request is refused (for example a blocked address), an error toast reports it.
...
`GET /api/addons/knowledge/connections-graph` returns a force-directed view of the current drive, drawn on its own page, `/drive/{drive}/addons/knowledge/connections`. The Notes page links to it at the bottom, and so does the **Related** tab of every Markdown or text file (**See connections as a graph**, after the file's relations). Following that link opens the graph centred on the file you came from, as if you had chosen **Center on this** for it; **Reset to full view** (or Escape) widens it to the whole drive. A file with no connections is not in the graph, so the page says so and shows the whole drive instead. It unions two edge sources:
