...
| `POST /filter-file-ids` | Body `{file_ids: [], trust_tier?}` → `{accessible: []}`. Drops IDs the caller can't see. Pass `trust_tier` (`verified` \| `unverified`) to additionally narrow to that tier — grounding surfaces send `verified` so unverified sources stop acting as evidence. Omit it and behaviour is unchanged. An unknown value returns 422. |
