...
| `GET` | `/api/files/{id}/thumbnail` | JPEG, 320px on the long edge. A picture keeps its own proportions, so the size varies; video and PDF are a fixed 320x180. Falls back to a placeholder image when the file has no generated thumbnail. Served for trashed and missing files too. |
