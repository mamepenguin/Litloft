...
| `GET` | `/api/drives/{drive}/folder-tree?root=&type_filter=&depth=1&flat=` | Lazy-expandable tree for the 2-pane browser. Default mode returns one level under `root` (subfolders plus depth-1 files matching `type_filter`, which takes the same values as the listing's `type`), each folder carrying `file_count` and `has_children`. `flat=true` returns a flattened subtree, capped at 50,000 entries. `depth` currently accepts only `1`. |
| `GET` | `/api/drives/{drive}/files?path=&recursive=&search=&favorite=&liked=&tag=&type=&sort=&order=&page=&limit=` | List files with filters. `path` is an exact `folder_path` match (direct children); `recursive=true` widens it to the whole subtree, and a recursive query with an empty `path` covers the drive. `search` matches title **or** folder path (each item carries a `match_source` of `filename` / `path` / `both`). `liked=true` selects files carrying a like stamp (`liked=false` the rest). `sort` is one of `created_at`, `title`, `file_size`, `liked_at`, `random`. `type` is one of `video`, `image`, `audio`, `document`, `archive`, `other`, `markdown`, `pdf` — the same vocabulary the tree's `type_filter` takes, applied by the same classifier. `markdown` and `pdf` are refinements of `document`, so `document` returns them as well; both are recognised by mime and, for rows whose mime was never recorded, by filename extension. Anything else is a 422. |
...
| `GET` | `/api/files/{id}/neighbors?sort=&order=` | Previous / next file id within the same folder under the given ordering, for player and viewer navigation. Also returns `position` (1-origin place in that ordering) and `total` (active files in the folder), for an `n / N` readout. `position` is `null` when the sort key cannot order the file — an unliked file under `sort=liked_at` — and `total` still counts the folder in that case. **Under `sort=liked_at` the two count different populations**: `position` ranks a file only against the other liked files, while `total` is the whole folder, so a liked file in a mostly-unliked folder reports a low position against a large total. A caller rendering `n / N` should read `total` as "files in this folder", not "files this ordering can reach". `total` is also folder-scoped only: it ignores every listing filter (`liked`, `type`, `tag`, `search`) and every `recursive=true`, so a viewer opened from a filtered listing walks a shorter sequence than `total` names. `prev_id` / `next_id` have always had the same scope. |
