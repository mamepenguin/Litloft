...
| `GET` | `/api/drives/{drive}/tags?folder_path=&type=` | Tag names with usage counts. `folder_path` scopes the counts to that folder's subtree. `type` (same values as the listing's `type`) counts only active files of that kind and omits tags none of them carry. |
...
| `GET` | `/api/files/{id}/relations?kind=` | Related files via `file_relations`, both directions, newest first. A counterpart related in both directions is listed once per `kind`. Trashed counterparts are dropped; missing ones are kept so the UI can grey them out. |
