...
| `data/relations_origin_v1_done` | The startup pass that resyncs every active Markdown file's links before the backend serves requests, once every file was readable. Never removed. | Itself | Keeps the one-time relation backfill from running on every boot; while any Markdown file is unreadable (an unmounted drive, a file over 1 MB), the pass repeats on the next start |
