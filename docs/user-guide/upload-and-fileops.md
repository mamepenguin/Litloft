...
Drop files anywhere on a page that lists a folder's contents, or pick **Files** or **Folder** from the **Add** menu. The drive home takes a drop too; what lands there goes to the drive root.
...
Batch operations work the same way: turn on select mode, pick the files, then choose the action from the selection bar. Batch copy, move and rename all report per-file errors rather than failing the whole run: one file that cannot be written does not stop the others.
