...
- **On a phone** the inspector is a bottom sheet with the same contents, and it rests rather than closes. While it is down, a strip along the bottom carries the file's name and the four buttons that act on it — like, favourite, **AI**, and the `⋮` menu — so those are in the same place on every file without opening anything. Raise the sheet and the strip goes: the sheet takes its place, and the same name and buttons are at the top of the sheet's own column instead. The rest of the file's details, including its trust state, are in the sheet itself. The same toggle raises it to half the screen, and the handle drags it to full; swiping down, tapping outside or pressing Escape sends it back to the strip rather than away.
- **The sheet scrolls as one column**, unlike the column on a wide screen: the title, the facts under it and the tags scroll away with everything else, and scrolling back to the top of the sheet is how you reach them again, because the strip along the bottom is not drawn while the sheet is up. Only the tab strip stays: once you have scrolled down to it, it stays at the top of the sheet, so you can switch tabs from anywhere in a long transcript. Raised to half the screen the sheet opens on the top of that column and you scroll to the tabs; raised to full it shows enough to read one.
