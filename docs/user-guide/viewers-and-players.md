...
- **On a phone** the inspector is a bottom sheet with the same contents, and it rests rather than closes. While it rests, a strip along the bottom carries the file's name and the four buttons that act on it — like, favourite, **AI**, and the `⋮` menu — so those are in the same place on every file without opening anything. Raise the sheet and the strip goes with it: the same name and the same four buttons are at the head of the sheet's own column, drawn in one place or the other and never in both. The rest of the file's details, including its trust state, are in the sheet itself. The same toggle raises it, and the handle drags it to full; swiping down, tapping outside or pressing Escape sends it back to the strip rather than away.
