...
- **Settings sheet** — captions on/off and playback speed (0.5, 0.75, 1, 1.25, 1.5, 2) are core's own. The player's owner contributes rows of its own to the same sheet, which core keeps opaque: the native player adds Picture-in-Picture, autoplay, a hand-back-to-browser-controls switch, and a subtitle track picker when the file has more than one track. On an iPhone — in Safari, from the home screen, or in the [iOS app](ios-app.md) — it also adds **Open in the iOS player**, which shows the video in iOS's own full-screen player; the page's controls pick up where that player left the video.
