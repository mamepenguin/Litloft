...
**Raising the sheet stops at the bottom of the player.** The first stop the toggle opens is not a fixed half of the screen — it is whatever is left below the player, so the whole picture and the controls under it stay visible and the sheet takes the rest. A wide clip therefore gets you more of the transcript than a tall one, and a clip shot in portrait on a small phone leaves almost nothing: there the sheet opens no further than the strip's own height, and reading it means dragging up over the video. On a page with no player — a note, a PDF, a photograph — the first stop is half the screen, as before.
