...
- **On a phone** it is a bottom sheet with the same contents, and it rests rather than closes. A 56px strip along the bottom always carries the file's name and the buttons that act on it — like, favourite, **AI**, and the `⋮` menu — so those are in the same place on every file without opening anything. The same toggle raises the sheet to half the screen, and you can drag it to full; swiping it down, tapping outside it or pressing Escape sends it back to the strip rather than away.
