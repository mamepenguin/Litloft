...
- **Add** anything to the folder, from the one **Add** button: upload files, upload a folder, create a folder. It is the only filled button on the bar. Addons contribute further rows below a separator at the bottom of the menu — with *knowledge* installed, **New note**; with *media_import*, **Import from URL**.
