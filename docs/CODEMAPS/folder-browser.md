...
| Path | Change |
|---|---|
| `backend/app/routers/drives.py` (`POST /api/drives/{drive_name}/files`, `create_text_file`) | Accepts any extension and opaque UTF-8 content. Default `conflict_mode=rename` auto-suffixes active/trashed collisions and preserves missing-state recovery. Opt-in `conflict_mode=error` returns 409 for any DB/filesystem collision without creating a suffix. The 1 MB cap and traversal checks are unchanged. |
| `backend/app/routers/drives.py` (`GET /api/drives/{drive_name}/files/by-path`) | Resolves one active file by exact NFC-normalized relative path. This is the non-paginated primitive used when a caller must distinguish “missing” from an exact existing destination. |
