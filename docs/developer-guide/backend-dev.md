...
- Thumbnails written before the picture box are replaced as each drive is scanned, one file at a time (`_is_letterboxed_image_thumbnail`). Both shapes render, so a part-migrated drive is not a broken one — though on the rows where a justified cell is not the picture's own ratio (a line at the stretch ceiling, a picture outside the ratio stops) the older thumbnail still shows some of its bars. A file that is missing from disk keeps its old thumbnail until it comes back — there is no source to regenerate from.
