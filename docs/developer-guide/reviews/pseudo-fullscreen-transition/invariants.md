...
5. The `getBoundingClientRect().top` of the first element after the player's wrapper does not change on any painted frame where the page is visible, during an entry or an exit, with and without a sibling row in the wrapper, and with reduced motion; and once pinned and settled it is where it was inline. *(Revised after r1, F1: previously "between inline and pinned". Revised after r2, F1: "where the page is visible" and the settled clause, because VideoPlayer's pre-existing one-commit lag moves the page under the opaque, viewport-sized frame for one frame under reduced motion.)*
